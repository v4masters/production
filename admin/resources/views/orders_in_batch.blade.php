                    ...
                    <div class="card mb-4">
                                ...
                                <div class="table-responsive text-nowrap">
                                    ...
                                    <table id="mytable" class="table table-striped" data-toggle="table" data-toolbar="#toolbar" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-fullscreen="false" data-show-columns="true" data-show-columns-toggle-all="true" data-detail-view="false" data-show-export="true" data-click-to-select="true" data-detail-formatter="detailFormatter" data-minimum-count-columns="2" data-show-pagination-switch="true" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, 500, 1000 ,all]" data-show-footer="true" data-response-handler="responseHandler">
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                            $i+1;
                                            if($data[$i]['ordersinfo']->ship_address_type==1){{$ship_address_type="<b>Home</b><br>";}} else {{$ship_address_type='<b>School</b><br>';}} 
                                            ...
                                            <tr>
                                                ...
                                                <td> @if($data[$i]['ordersinfo']->tracking_status!=""){!!$data[$i]['ordersinfo']->tracking_status!!}@endif @if($data[$i]['ordersinfo']->print_status==1)<span class="bg-success text-white p-1">{{'Printed';}}</span> @else <span class="bg-danger text-white p-1">{{'Unprint';}}</span>@endif</td>
                                                ...
                                                @if($data[$i]['ordersinfo']->order_status==3 && array_diff($data[$i]['ordersinfo']->shipped_status, [3,4])) 
                                                  
                                                  <button type="button" onclick="deliver_order_shipped_item(`{{$data[$i]['ordersinfo']->invoice_number;}}`,`{{$data[$i]['ordersinfo']->vendor_id;}}`)" class="btn-sm mt-3 btn btn-danger">Deliver</button>
                                                 
                                                @elseif($data[$i]['ordersinfo']->order_status==4 && $data[$i]['ordersinfo']->shipped_status[0]==5)    
                                                  
                                                  <button type="button" class="btn btn-success btn-sm fw-5">Deliverd</button>
                                                 
                                                @else
                                                
                                                  <span class="btn btn-warning btn-sm fw-5">Item Pending</span>
                                                
                                                @endif
                                                
                                                <br>

                                                 <a href="{{url('order_process_status',[$data[$i]['ordersinfo']->invoice_number,$data[$i]['ordersinfo']->vendor_id])}}" class="btn-sm mt-3 btn btn-primary">Update Status</a>
                      ...
                      <div id="batch_order_bill" style="color:#000 !important">
                              ...
                              <div  style="color:#000 !important;page-break-before: always !important;">
                                        ...
                                        <div style="border:1px solid #000;width: 200mm;height: 261mm;margin: 0 auto;padding: 20px;box-sizing: border-box;">
                                            ...
                                            <div style="padding-bottom:8px;text-align: right;border-bottom: 1px solid rgb(189, 180, 180);font-size: small;line-height: 0.6;">
                                                ...
                                                <p><span style="font-weight: bold">Final Price:</span> {{$data[$j]['ordersinfo']->grand_total}}</p>
...
function deliver_order_shipped_item(order_id,vendor_id)
{
    $(".loader").css("display","block");
    $.ajax({
                type: "POST",
                url: "{{url('/') }}/deliver_order",
                data: {
                    "_token": "{{ csrf_token() }}",
