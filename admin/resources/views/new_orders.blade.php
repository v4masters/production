                                        ...
                                        <thead>
                                         ...
                                         @foreach($neworders as $key => $data)
                                            ...
                                            <tr>
                                                <td>{{$key}}</td>
                                                <td>{!!$data[1][0]!!}</td>
                                                <td><p  class=" mb-1 py-1 px-1 border border-1"  ><b>OID - </b><span onclick="copy(this)">{{$data[0]->invoice_number}}</span></p><p  class="py-1 px-1 border border-1"  ><b>TID - </b><span  onclick="copy(this)">{{$data[0]->transaction_id}}</span></p><b>Time - </b>{{$data[0]->transaction_date}}</td>
                                                <td><b>{{$data[1][1]}}</b><br>@if($data[0]->mode_of_payment==1){{'Online'}}@else{{'COD'}}@endif</td>
                                                <td><b>{{$data[0]->name}}</b><br>{{$data[0]->phone_no}}<br>{{$data[0]->school_code}}<br>{{$data[0]->district.','.$data[0]->city}}<br>{{$data[0]->address}}<br>{{$data[0]->post_office.','.$data[0]->pincode}}</td>
                                               
                                                <td>
                                                    <button type="button" onclick="getOrderDetail(`{{$data[0]->invoice_number}}`,`{{$data[1][2]}}`)" class="btn-sm btn btn-primary">View</button><br><br>
                                                    <!--<button type="button" onclick="cancle_order(`{{$data[0]->invoice_number}}`)" class="btn-sm btn btn-danger">Cancle</button>-->
                                                </td>
                                            </tr>
...
inp.remove();
      ...
      //getOrderDetail
        function getOrderDetail(Orderid,vendor_id) {
...
//   <button type="button" onclick="accept_order(`'+response.order_info.invoice_number+'`)" class="btn btn-success">Accept Order</button>
