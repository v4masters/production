                                ...
                                <div class="list_wrapper">
                                    ...
                                    <div class="col-md-3 m-3">
                                        ...
                                        @if($images->dp_status == 1)
                                            <img class="mx-auto d-flex img-fluid border border-success border-3" src="{{Storage::disk('s3')->url($images->folder.'/' .$images->image)}}">
                                             Active Banner Image
                                        @else
                                            <img class="mx-auto d-flex img-fluid" src="{{Storage::disk('s3')->url($images->folder.'/' .$images->image)}}">
                                            <input class="btn btn-primary form-control mt-1" type="button"  onclick="dp_status_changed(`{{$images->id}}`,`{{$images->item_id}}`)"  value="Set as Main Banner" >
                                        @endif
  ...
  function removeinvimge(id)
   ...
   function dp_status_changed(image_id,item_id){
        $(".loader").css("display","block");
                $.ajax({
                type: "POST",
                url: "{{url('/') }}/updateapproveddpstatus",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "image_id": image_id,
                    "item_id": item_id
