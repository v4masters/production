...
<head>
    ...
    <title>New Pickup Point Orders</title>
            ...
            <div class="layout-page">
                ...
                <div class="container mt-3">
                    <div class="card mb-4">
                        ...
                        <div class="container">
                            <div class="card">
                                <h5 class="card-header">View  Pickup Point New Order</h5>
                                    ...
                                    <table class="table table-striped" data-toggle="table" data-toolbar="#toolbar" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-fullscreen="false" data-show-columns="true" data-show-columns-toggle-all="true" data-detail-view="false" data-show-export="true" data-click-to-select="true" data-detail-formatter="detailFormatter" data-minimum-count-columns="2" data-show-pagination-switch="true" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, 500, 1000 ,all]" data-show-footer="true" data-response-handler="responseHandler">
                                        <thead>
                                            <tr>
                                                ...
                                                <th data-sortable="true">Pickup <br>Point</th>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach($neworders as $key => $data)
                                            @php
                                            $key++;
                                            @endphp
                                            <tr>
                                                <td>{{$key}}</td>
                                                <td><p  class=" mb-1 py-1 px-1 border border-1"  ><b>OID - </b><span onclick="copy(this)">{{$data->invoice_number}}</span></p><p  class="py-1 px-1 border border-1"  ><b>TID - </b><span  onclick="copy(this)">{{$data->transaction_id}}</span></p><b>Time - </b>{{$data->transaction_date}}</td>
                                                 <td><b>{{$data->pp_name}}</b></td>
                                               
                                                <td><b>{{$data->vendor_total_amount}}</b><br>@if($data->mode_of_payment==1){{'Online'}}@else{{'COD'}}@endif</td>
                                                <td><b>{{$data->name}}</b><br>{{$data->phone_no}}<br>{{$data->school_code}}<br>{{$data->district.','.$data->city}}<br>{{$data->address}}<br>{{$data->post_office.','.$data->pincode}}</td>
                                                <td>
      ...
      //getOrderDetail
        function getOrderDetail(Orderid) {
            ...
            $.ajax({
                ...
                var tabledata=' <table id="table" class="table table-striped"><thead> <tr><th>#</th><th >ItemCode</th><th>Itemname</th><th>Cat<th>Rate</th><th >Qty</th></tr></thead> <tbody>';
                 for(var i=0;i<response.item_info.length;i++)
				 {
				     const id=i+1;
				     tabledata+='<tr><td>'+id+'</td><td>'+response.item_info[i].item_id+'</td><td>'+response.item_info[i].itemname+'</td><td>'+response.item_info[i].cat+'</td><td>'+response.item_info[i].rate+'</td><td>'+response.item_info[i].qty+'</td></tr>';
                  
                 }
                 tabledata+=' </tbody></table>';
                 $('#itemdiv').append(tabledata);
            
                 var amount_div='<div class="row"><div class="col-md-9"><p class="amountdiv"><b>Total Amount: </b>'+response.order_info.total_amount+'</p></div><div class="col-md-3"><button type="button" onclick="accept_order(`'+response.order_info.invoice_number+'`)" class="btn btn-success">Accept Order</button></div></div>';
