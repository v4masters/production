...
<head>
    ...
    <title>Pickup Point COD Orders Under Process </title>
                ...
                <div class="container mt-3">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="container">
                         ...
                         <h5 class="card-header">View Pickup Point COD Order Under Process</h5>
                        <div class="row">
                         <div class="col-md-5">
                         <div class="input-group">     
                         <select class="form-select" id="pickup_point_select" >
                                        <option selected disabled value="">Select</option>
                                        <option value="0">NULL</option>
                                         @foreach($pp_data as  $pp_data_name)
                                         
                                         @if(request()->segment(2)==$pp_data_name->id)
                                         <option value="{{$pp_data_name->id}}" selected >{{$pp_data_name->name}}</option>
                                         @else
                          ...
                          <button type="button" onclick="get_pp_orders()" class="btn btn-secondary">get</button>
                          </div>
                          </div>
                          </div>
                          ...
                          <form method="post" id="myform" action="{{url('/') }}/create_batch" enctype="multipart/form-data" novalidate>
                                        ...
                                        <input type='hidden' value='1' name='pp_status'>
                                    ...
                                    <table id="mytable" class="table table-striped" data-toggle="table" data-toolbar="#toolbar" data-search="true" data-show-refresh="true" data-show-toggle="true" data-show-fullscreen="false" data-show-columns="true" data-show-columns-toggle-all="true" data-detail-view="false" data-show-export="true" data-click-to-select="true" data-detail-formatter="detailFormatter" data-minimum-count-columns="2" data-show-pagination-switch="true" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, 500, 1000 ,all]" data-show-footer="true" data-response-handler="responseHandler">
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th data-sortable="true">School <br>Code</th>
                                        ...
                                        <tbody>
                                            ...
                                            @endphp
                                            <tr>
                                                <td>{{$key}} <input class="form-check-input" type='checkbox' onclick='selectsinglecheckbox()' name='invoice_number[]' value='{{$data->invoice_number}}'></td>
                                                <td>{{$data->pp_name}}</td>
                                                ...
                                                <td class="add_td">{{$data->school_code}}</td>
                                              
                                                <td> @if($data->tracking_status!=""){!!$data->tracking_status!!}@endif @if($data->print_status==1)<span class="bg-success text-white p-1">{{'Printed';}}</span> @else <span class="bg-danger text-white p-1">{{'Unprint';}}</span>@endif</td>
                                                <td><a  href="{{url('pp_order_process_status',$data->invoice_number)}}" class="btn btn-primary">View<br> Order</a></td>
...
function get_pp_orders()
{
    let pickup_point_select=document.getElementById('pickup_point_select').value;
     setTimeout(function()
     {
      window.location.href="{{url('/') }}/pp_order_under_process_cod_pp/"+pickup_point_select;
     }, 2000);
}
