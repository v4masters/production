...
use App\Models\InventoryModelVendor;
 ...
 public function add_inventory_catalog(Request $request)
  {
    ...
    public function add_all_inventory(Request $request)
    {
          ...
          if(isset($request->product_name)){$product_name=$request->product_name;}else{$product_name=NULL;}
          if(isset($request->description)){$description=$request->description;}else{$description=NULL;}
          if(isset($request->author)){$author=$request->author;}else{$author=NULL;}
          if(isset($request->book_format)){$book_format=$request->book_format;}else{$book_format=NULL;}
          if(isset($request->youtube_url)){$youtube_url=$request->youtube_url;}else{$youtube_url=NULL;}
          if(isset($request->printer_details)){$printer_details=$request->printer_details;}else{$printer_details=NULL;}
          if(isset($request->publish_year)){$publish_year=$request->publish_year;}else{$publish_year=NULL;}
          if(isset($request->importer_detail)){$importer_detail=$request->importer_detail;}else{$importer_detail=NULL;}
          if(isset($request->laptop_capacity)){$laptop_capacity=$request->laptop_capacity;}else{$laptop_capacity=NULL;}
          if(isset($request->material)){$material=$request->material;}else{$material=NULL;}
          if(isset($request->warranty)){$warranty=$request->warranty;}else{$warranty=NULL;}
          if(isset($request->water_resistant)){$water_resistant=$request->water_resistant;}else{$water_resistant=NULL;}
          if(isset($request->type)){$type=$request->type;}else{$type=NULL;}
          if(isset($request->country_of_origin)){$country_of_origin=$request->country_of_origin;}else{$country_of_origin=NULL;}
          if(isset($request->manufacturer_detail)){$manufacturer_detail=$request->manufacturer_detail;}else{$manufacturer_detail=NULL;}
          if(isset($request->packer_detail)){$packer_detail=$request->packer_detail;}else{$packer_detail=NULL;}
          if(isset($request->material_type)){$material_type=$request->material_type;}else{$material_type=NULL;}
          if(isset($request->paper_finish)){$paper_finish=$request->paper_finish;}else{$paper_finish=NULL;}
          if(isset($request->paper_size)){$paper_size=$request->paper_size;}else{$paper_size=NULL;}
          if(isset($request->weight_unit)){$weight_unit=$request->weight_unit;}else{$weight_unit=NULL;}
          if(isset($request->product_weight)){$product_weight=$request->product_weight;}else{$product_weight=NULL;}
          if(isset($request->fssai_license_number)){$fssai_license_number=$request->fssai_license_number;}else{$fssai_license_number=NULL;}
          if(isset($request->shelf_life)){$shelf_life=$request->shelf_life;}else{$shelf_life=NULL;}
          if(isset($request->veg_nonveg)){$veg_nonveg=$request->veg_nonveg;}else{$veg_nonveg=NULL;}
          if(isset($request->product_length)){$product_length=$request->product_length;}else{$product_length=NULL;}
          if(isset($request->product_type)){$product_type=$request->product_type;}else{$product_type=NULL;}
          if(isset($request->product_unit)){$product_unit=$request->product_unit;}else{$product_unit=NULL;}
          if(isset($request->character)){$character=$request->character;}else{$character=NULL;}
          if(isset($request->no_of_compartment)){$no_of_compartment=$request->no_of_compartment;}else{$no_of_compartment=NULL;}
          if(isset($request->backpack_style)){$backpack_style=$request->backpack_style;}else{$backpack_style=NULL;}
          if(isset($request->bag_capacity)){$bag_capacity=$request->bag_capacity;}else{$bag_capacity=NULL;}
          if(isset($request->gender)){$gender=$request->gender;}else{$gender=NULL;}
          if(isset($request->pattern)){$pattern=$request->pattern;}else{$pattern=NULL;}
          if(isset($request->recommended_age)){$recommended_age=$request->recommended_age;}else{$recommended_age=NULL;}
          if(isset($request->fabric)){$fabric=$request->fabric;}else{$fabric=NULL;}
          if(isset($request->fit)){$fit=$request->fit;}else{$fit=NULL;}
          if(isset($request->neck)){$neck=$request->neck;}else{$neck=NULL;}
          if(isset($request->sleeve_length)){$sleeve_length=$request->sleeve_length;}else{$sleeve_length=NULL;}
          if(isset($request->stitch_type)){$stitch_type=$request->stitch_type;}else{$stitch_type=NULL;}
          if(isset($request->bottom_type)){$bottom_type=$request->bottom_type;}else{$bottom_type=NULL;}
          if(isset($request->bottomwear_color)){$bottomwear_color=$request->bottomwear_color;}else{$bottomwear_color=NULL;}
          if(isset($request->bottomwear_fabric)){$bottomwear_fabric=$request->bottomwear_fabric;}else{$bottomwear_fabric=NULL;}
          if(isset($request->kurta_color)){$kurta_color=$request->kurta_color;}else{$kurta_color=NULL;}
          if(isset($request->kurta_fabric)){$kurta_fabric=$request->kurta_fabric;}else{$kurta_fabric=NULL;}
          if(isset($request->length)){$length=$request->length;}else{$length=NULL;}
          if(isset($request->set_type)){$set_type=$request->set_type;}else{$set_type=NULL;}
          if(isset($request->occasion)){$occasion=$request->occasion;}else{$occasion=NULL;}
          if(isset($request->ornamentation)){$ornamentation=$request->ornamentation;}else{$ornamentation=NULL;}
          if(isset($request->pattern_type)){$pattern_type=$request->pattern_type;}else{$pattern_type=NULL;}
          if(isset($request->sleeve_styling)){$sleeve_styling=$request->sleeve_styling;}else{$sleeve_styling=NULL;}
          if(isset($request->sole_material)){$sole_material=$request->sole_material;}else{$sole_material=NULL;}
          if(isset($request->connectivity)){$connectivity=$request->connectivity;}else{$connectivity=NULL;}
          if(isset($request->operating_system)){$operating_system=$request->operating_system;}else{$operating_system=NULL;}
          if(isset($request->ram)){$ram=$request->ram;}else{$ram=NULL;}
          if(isset($request->warranty_service_type)){$warranty_service_type=$request->warranty_service_type;}else{$warranty_service_type=NULL;}
          if(isset($request->dual_camera)){$dual_camera=$request->dual_camera;}else{$dual_camera=NULL;}
          if(isset($request->expandable_storage)){$expandable_storage=$request->expandable_storage;}else{$expandable_storage=NULL;}
          if(isset($request->headphone_jack)){$headphone_jack=$request->headphone_jack;}else{$headphone_jack=NULL;}
          if(isset($request->internal_memory)){$internal_memory=$request->internal_memory;}else{$internal_memory=NULL;}
          if(isset($request->no_of_primary_camera)){$no_of_primary_camera=$request->no_of_primary_camera;}else{$no_of_primary_camera=NULL;}
          if(isset($request->no_of_secondary_camera)){$no_of_secondary_camera=$request->no_of_secondary_camera;}else{$no_of_secondary_camera=NULL;}
          if(isset($request->primary_camera)){$primary_camera=$request->primary_camera;}else{$primary_camera=NULL;}
          if(isset($request->screen_length_size)){$screen_length_size=$request->screen_length_size;}else{$screen_length_size=NULL;}
          if(isset($request->secondary_camera)){$secondary_camera=$request->secondary_camera;}else{$secondary_camera=NULL;}
          if(isset($request->sim)){$sim=$request->sim;}else{$sim=NULL;}
          if(isset($request->sim_type)){$sim_type=$request->sim_type;}else{$sim_type=NULL;}
          if(isset($request->add_ons)){$add_ons=$request->add_ons;}else{$add_ons=NULL;}
          if(isset($request->body_material)){$body_material=$request->body_material;}else{$body_material=NULL;}
          if(isset($request->burner_material)){$burner_material=$request->burner_material;}else{$burner_material=NULL;}
          if(isset($request->no_of_burners)){$no_of_burners=$request->no_of_burners;}else{$no_of_burners=NULL;}
          if(isset($request->packaging_breadth)){$packaging_breadth=$request->packaging_breadth;}else{$packaging_breadth=NULL;}
          if(isset($request->packaging_height)){$packaging_height=$request->packaging_height;}else{$packaging_height=NULL;}
          if(isset($request->packaging_length)){$packaging_length=$request->packaging_length;}else{$packaging_length=NULL;}
          if(isset($request->packaging_unit)){$packaging_unit=$request->packaging_unit;}else{$packaging_unit=NULL;}
          if(isset($request->product_breadth)){$product_breadth=$request->product_breadth;}else{$product_breadth=NULL;}
          if(isset($request->product_height)){$product_height=$request->product_height;}else{$product_height=NULL;}
    ...
    //   "size_chart" => $request->size_chart, 
         ...
         if(isset($request->brand))
              {
                 ...
                 $checkbrandexist= managemasterbrandModel::where(['id'=>$request->brand,"del_status"=>0])->count();
                 if($checkbrandexist==1)
                 {
                    $brand=$request->brand;
        ...
        if(isset($request->cat_type)){$colname='class';}else{$colname="color";}
        $resrow=0;
        $size = $request->size;
        ...
        for ($s = 0; $s <$count ; $s++) {
         ...
         if(isset($request->$redition)){    $edition= $request->$redition;}else{$edition=[];}
         
         
         if($request->file('book_pdf'.$i)!=NULL){   $book_pdf = $request->file('book_pdf'.$i);}else{$book_pdf=[];}
            ...
            for($j=0;$j<count($subclass);$j++) {
              ...
              $lastprodata = InventoryModelVendor::orderBy('id', 'DESC')->first();
              $product_id=rand(10,100000).$lastprodata->id+1;
        
               
                if(isset($book_pdf[$j])) {
                    $book_pdfdata =$this->upload_image($book_pdf[$j],'inventory_vendor/pdf/');
                } else {
                    $book_pdfdata = "";
                }
                 ...
                 if(isset($net_quantity[$j])){$newnet_quantity=$net_quantity[$j];}else{$newnet_quantity=NULL;}
                 if(isset($min_quantity[$j])){$newmin_qyt=$min_quantity[$j];}else{$newmin_qyt=NULL;}
                 if(isset($qty_unit[$j])){$newqty_unit=$qty_unit[$j];}else{$newqty_unit=NULL;}
                 if(isset($class_hsncode[$j])){$newhsncode=$class_hsncode[$j];}else{$newhsncode=NULL;}
                 if(isset($class_barcode[$j])){$newbarcode=$class_barcode[$j];}else{$newbarcode=NULL;}
                 if(isset($class_isbn[$j])){$newisbn=$class_isbn[$j];}else{$newisbn=NULL;}
                 if(isset($class_net_weight[$j])){$newnet_weight=$class_net_weight[$j];}else{$newnet_weight=NULL;}
                 if(isset($mrp[$j])){$newmrp=$mrp[$j];}else{$newmrp=NULL;}
                 if(isset($discount[$j])){$newdiscount=$discount[$j];}else{$newdiscount=NULL;}
                 if(isset($discounted_price[$j])){$newdiscounted_price=$discounted_price[$j];}else{$newdiscounted_price=NULL;}
                 if(isset($sales_price[$j])){$newsales_price=$sales_price[$j];}else{$newsales_price=NULL;}
                 if(isset($gst[$j])){$newgst=$gst[$j];}else{$newgst=NULL;}
                 if(isset($shipping_charges[$j])){$newshipping_charges=$shipping_charges[$j];}else{$newshipping_charges=NULL;}
                 if(isset($edition[$j])){$newedition=$edition[$j];}else{$newedition=NULL;}
                ...
                $res=InventoryModelVendor::create($subdata);
                $lastinsertid=$res->id;
                ...
                for($m=0;$m<$countimg;$m++)
                {
                  ...
                  if (isset($pro_imgname[$m])) {
                    $pro_imgnameorg =  $this->upload_image($pro_imgname[$m],'inventory_vendor/');
                    } 
                    else 
                    {
                    $pro_imgnameorg = "";
                    }
                    if($m==0){$dp_status=1;}else{$dp_status=0;}
                        $subdataimg = [
                          'vendor_id'=>session('id'),
                  ...
                  $subresimg = InventoryVenImgModel::create($subdataimg);
       ...
       public function view_pending_inventory()
    {
        $array=array( 'vendor_id'=>session('id'),'status'=>0,'del_status'=>0);
        
        $data = InventoryModelVendor::orderBy('id', 'DESC')->where($array)->get();
        return view('inventory_pending', ['pagedata' => $data]);
    }
    ...
    {
        $array=array( 'vendor_id'=>session('id'),'status'=>1,'del_status'=>0);
        
        $data = InventoryformsModel::orderBy('id', 'DESC')->where($array)->get();
        return view('inventory_approved', ['pagedata' => $data]);
    }



    public function inventorydetailapproved(string $id)
    {
        
       $where = array('inventory_new.id' => $id,'vendor_id'=>session('id'));
        ...
        $cat_four = managesubcategorythreeModel::select('title')->where('id', $data->inv_cat_four)->first();
         ...
         $inv_images = InventoryImgModel::select('image','alt','folder')->where('item_id', $data->id)->get();
   ...
   public function inventorydetailpending(string $id)
    {
        
       $where = array('inventory_vendor.id' => $id,'vendor_id'=>session('id'));
        ...
        $cat_four = managesubcategorythreeModel::select('title')->where('id', $data->inv_cat_four)->first();
         ...
         $inv_images = InventoryVenImgModel::select('image','alt','folder')->where('item_id', $data->id)->get();
    ...
    //delete inventory
    public function delete_app_inventory(Request $request)

    {

        $data = InventoryformsModel::where(['id'=> Request('id'),'vendor_id'=>session('id')]);
  ...
  //delete inventory
    public function delete_pen_inventory(Request $request)

    {

        $data = InventoryModelVendor::where(['id'=> Request('id'),'vendor_id'=>session('id')]);
...
//view full inventory 
    ...
    {
        $array=array('vendor_id'=>session('id'),'del_status'=>0);
    ...
    //delete view  inventory
    public function delete_view_inv(Request $request)
...
public function updateinventorydpstatus(Request $request){
    $where=['vendor_id'=>session('id'),'item_id'=>$request->item_id];
...
//update_net_quantity
    ...
    public function updateappinventorydpstatus(Request $request){
...
{
    $array=array('status'=>1,'del_status'=>0,'vendor_id'=>session('id'));
    $data = InventoryformsModel::orderBy('id', 'DESC')->where($array)->where('qty_available', '<=', 0)->get();
        for($i=0; $i<count($data); $i++){
            $vendor = VendorModel::where(['unique_id'=>$data[$i]->vendor_id])->first();
            $data[$i]->vendor_info = $vendor;
